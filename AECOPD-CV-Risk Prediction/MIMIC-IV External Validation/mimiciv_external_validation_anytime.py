...
def extract_first_labs_anytime(original_df: pd.DataFrame, mimic_root: Path, hours: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # No fixed upper time window: retain the first plausible value after admission.
    # If dischtime is available, values after discharge are excluded below.
...
def run_validation(df: pd.DataFrame, outdir: Path) -> dict:
    ...
    roc_rows = make_roc_plot(y, p_original, roc_path, outdir / "table_roc_external_validation_anytime.csv")
    cal = make_calibration_plot(y, p_original, cal_path, outdir / "table_calibration_external_validation_anytime.csv")
    make_dca_plot(y, p_original, dca_path, outdir / "table_decision_curve_external_validation_anytime.csv")
    make_quartile_plot(y, p_original, quart_path, outdir / "table_quartiles_external_validation_anytime.csv")
...
def main() -> None:
    ...
    # Remove old prediction columns from the original validation file before adding the new anytime-lab predictions.
