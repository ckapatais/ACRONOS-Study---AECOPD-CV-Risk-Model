...
def extract_first_labs_24h(original_df: pd.DataFrame, mimic_root: Path, hours: int = 12) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for row in cohort_times.itertuples(index=False):
        ...
        rows.append({
            ...
            "hadm_id": hadm,
            "ph_24h": ph.get("value", np.nan),
            "ph_24h_charttime": ph.get("charttime", pd.NaT),
            "ph_24h_itemid": ph.get("itemid", np.nan),
            "bun_24h": bun_value,
            "bun_24h_charttime": bun.get("charttime", pd.NaT),
            "bun_24h_itemid": bun.get("itemid", np.nan),
            "urea_24h": bun_value * BUN_TO_UREA_FACTOR if pd.notna(bun_value) else np.nan,
            "lactate_24h": lact.get("value", np.nan),
            "lactate_24h_charttime": lact.get("charttime", pd.NaT),
            "lactate_24h_itemid": lact.get("itemid", np.nan),
        })
...
def regenerate_predictions(df: pd.DataFrame, internal_model_dir: Path, medians_csv: str | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    X["ph"] = pd.to_numeric(df["ph_24h"], errors="coerce")
    X["urea"] = pd.to_numeric(df["urea_24h"], errors="coerce")
    X["lactate"] = pd.to_numeric(df["lactate_24h"], errors="coerce")

    pred = pd.DataFrame(index=df.index)
    for f in MODEL_FEATURES:
        pred[f"{f}_24h_missing_before_imputation"] = X[f].isna().astype(int)
        X[f] = X[f].fillna(medians[f])
        pred[f"{f}_model_input_24h"] = X[f].astype(float)
    ...
    pred["predicted_probability_original_24h"] = p_original
    pred["linear_predictor_frozen_24h"] = lp_frozen
    pred["predicted_probability_frozen_24h"] = p_frozen
...
def availability_table(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["ph_24h", "urea_24h", "lactate_24h"]
    rows = []
    for c in cols:
    ...
    rows.append({
        "variable": "all_three_24h_labs_complete",
...
def run_validation(df: pd.DataFrame, outdir: Path) -> dict:
    ...
    roc_rows = make_roc_plot(y, p_original, p_iso, roc_path, outdir / "table_roc_external_validation_24h.csv")
    cal = make_calibration_plot(y, p_iso, cal_path, outdir / "table_calibration_external_validation_24h.csv")
    make_dca_plot(y, p_iso, dca_path, outdir / "table_decision_curve_external_validation_24h.csv")
    make_quartile_plot(y, p_iso, quart_path, outdir / "table_quartiles_external_validation_24h.csv")
...
def main() -> None:
    ...
    with pd.ExcelWriter(outdir / "Database_MIMIC-IV_external_validation_24h_labs_predictions.xlsx", engine="openpyxl", mode="w") as writer:
        final_df.to_excel(writer, sheet_name="AECOPD_validation_dataset_24h", index=False)
        availability_table(final_df).to_excel(writer, sheet_name="lab_availability_24h", index=False)
    ...
    (outdir / "summary_external_validation_24h.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
