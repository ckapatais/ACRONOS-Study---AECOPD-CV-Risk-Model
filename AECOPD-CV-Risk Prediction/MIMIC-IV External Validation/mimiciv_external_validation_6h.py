...
def extract_first_labs_6h(original_df: pd.DataFrame, mimic_root: Path, hours: int = 6) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for row in cohort_times.itertuples(index=False):
        ...
        rows.append({
            ...
            "hadm_id": hadm,
            "ph_6h": ph.get("value", np.nan),
            "ph_6h_charttime": ph.get("charttime", pd.NaT),
            "ph_6h_itemid": ph.get("itemid", np.nan),
            "bun_6h": bun_value,
            "bun_6h_charttime": bun.get("charttime", pd.NaT),
            "bun_6h_itemid": bun.get("itemid", np.nan),
            "urea_6h": bun_value * BUN_TO_UREA_FACTOR if pd.notna(bun_value) else np.nan,
            "lactate_6h": lact.get("value", np.nan),
            "lactate_6h_charttime": lact.get("charttime", pd.NaT),
            "lactate_6h_itemid": lact.get("itemid", np.nan),
        })
...
def regenerate_predictions(df: pd.DataFrame, internal_model_dir: Path, medians_csv: str | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    X["ph"] = pd.to_numeric(df["ph_6h"], errors="coerce")
    X["urea"] = pd.to_numeric(df["urea_6h"], errors="coerce")
    X["lactate"] = pd.to_numeric(df["lactate_6h"], errors="coerce")

    pred = pd.DataFrame(index=df.index)
    for f in MODEL_FEATURES:
        pred[f"{f}_6h_missing_before_imputation"] = X[f].isna().astype(int)
        X[f] = X[f].fillna(medians[f])
        pred[f"{f}_model_input_6h"] = X[f].astype(float)
    ...
    pred["linear_predictor_original_6h"] = lp_original
    pred["predicted_probability_original_6h"] = p_original
    pred["linear_predictor_frozen_6h"] = lp_frozen
    pred["predicted_probability_frozen_6h"] = p_frozen
    ...
    metadata = {
        ...
        "medians": medians,
        "definition": "Original MIMIC-IV validation cohort/outcome preserved; only pH, urea, and lactate replaced by first plausible 6h labevents values, with remaining missing values median-imputed.",
    }
...
def run_validation(df: pd.DataFrame, outdir: Path) -> dict:
    ...
    roc_path = outdir / "figure_roc_external_validation_6h.png"
    dca_path = outdir / "figure_decision_curve_external_validation_6h.png"
    cal_path = outdir / "figure_calibration_external_validation_6h.png"
    quart_path = outdir / "figure_quartiles_external_validation_6h.png"

    roc_rows = make_roc_plot(y, p, roc_path, outdir / "table_roc_external_validation_6h.csv")
    cal = make_calibration_plot(y, p, cal_path, outdir / "table_calibration_external_validation_6h.csv")
    make_dca_plot(y, p, dca_path, outdir / "table_decision_curve_external_validation_6h.csv")
    make_quartile_plot(y, p, quart_path, outdir / "table_quartiles_external_validation_6h.csv")

    try:
        make_composite_figure(roc_path, dca_path, cal_path, quart_path, outdir / "External_Validation_Composite_6h.png")
    except Exception:
        pass
...
def main() -> None:
    ...
    labs6, item_table = extract_first_labs_6h(original, MIMIC_ROOT, hours=LAB_WINDOW_HOURS)
    item_table.to_csv(outdir / "lab_itemids_used_6h.csv", index=False)
    ...
    final_df.to_csv(outdir / "patient_level_external_validation_predictions_MIMIC-IV_6h_labs.csv", index=False)

    with pd.ExcelWriter(outdir / "Database_MIMIC-IV_external_validation_6h_labs_predictions.xlsx", engine="openpyxl", mode="w") as writer:
        final_df.to_excel(writer, sheet_name="AECOPD_validation_dataset_6h", index=False)
        availability_table(final_df).to_excel(writer, sheet_name="lab_availability_6h", index=False)
        item_table.to_excel(writer, sheet_name="lab_itemids_used", index=False)

    avail = availability_table(final_df)
    avail.to_csv(outdir / "lab_availability_6h.csv", index=False)
    ...
    outcome_counts.to_csv(outdir / "outcome_definition_check_6h.csv", index=False)
    ...
    pd.DataFrame([summary]).to_csv(outdir / "summary_external_validation_6h.csv", index=False)
    (outdir / "summary_external_validation_6h.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    print(f"AUC 6h labs: {summary['auc']:.3f} ({summary['auc_ci_low']:.3f}-{summary['auc_ci_high']:.3f})")
    print(f"Output: {outdir}")
    print("")
    print("Outcome definition check:")
    print(outcome_counts.to_string(index=False))
    print("")
    print("6h lab availability:")
