...
def extract_labs(lab: pd.DataFrame, ids: set[int], start_min: int, end_min: int | None, patient_window: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    lab = lab[lab["lab_offset"] >= start_min].copy()
    if patient_window is not None and "unitdischargeoffset" in patient_window.columns:
        window = patient_window[["patientunitstayid", "unitdischargeoffset"]].copy()
        window["patientunitstayid"] = pd.to_numeric(window["patientunitstayid"], errors="coerce")
        window["unitdischargeoffset"] = pd.to_numeric(window["unitdischargeoffset"], errors="coerce")
        window = window.dropna(subset=["patientunitstayid"]).drop_duplicates("patientunitstayid")
        lab = lab.merge(window, on="patientunitstayid", how="left")
        lab = lab[(lab["unitdischargeoffset"].isna()) | (lab["lab_offset"] <= lab["unitdischargeoffset"])].copy()
    elif end_min is not None:
        lab = lab[lab["lab_offset"] <= end_min].copy()
...
def build_dataset(eicu_dir: Path, lab_start_min: int, lab_end_min: int, medians: dict[str, float]) -> pd.DataFrame:
    ...
    labs = extract_labs(lab, ids, lab_start_min, lab_end_min, patient_window=patient)
    patient = patient[patient["patientunitstayid"].isin(ids)].copy()
    patient["age"] = prepare_age(patient)
    columns = ["patientunitstayid", "age"]
    for column in ["uniquepid", "gender", "hospitalid", "hospitaladmitoffset", "unitadmitoffset", "unitadmittime24", "unitdischargeoffset"]:
...
def write_lab_availability(data: pd.DataFrame, output: Path, lab_start_min: int, lab_end_min: int) -> None:
    row = {
        "lab_window": "ICU admission-discharge" if lab_end_min is None else f"{lab_start_min}-{lab_end_min} min",
        "lab_strategy": "first",
        "lab_selection_rule": "earliest available value from ICU admission until ICU discharge",
        "n": int(len(data)),
...
def main() -> None:
    ...
    summary = {
        ...
        "lab_window_minutes": "ICU admission-discharge" if args.lab_end_min is None else f"{args.lab_start_min}-{args.lab_end_min}",
        "lab_strategy": "first",
        "lab_selection_rule": "earliest available value from ICU admission until ICU discharge",
        "auc": float(roc["auc"]),
